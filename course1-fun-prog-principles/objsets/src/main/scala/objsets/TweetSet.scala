...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
    ...
    def union(that: TweetSet): TweetSet = {
      left.union(right).union(that).incl(elem) // left and right don't include initial elem, thus final incl
    }

    def mostRetweeted: Tweet = {
      // helper function needed to traverse list (like filterAcc)
      def findMax(tw1: Tweet, tw2: Tweet): Tweet= {
