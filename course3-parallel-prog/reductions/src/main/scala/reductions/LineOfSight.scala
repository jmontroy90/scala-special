...
object LineOfSight {
  ...
  def lineOfSight(input: Array[Float], output: Array[Float]): Unit = {
    input.zipWithIndex.drop(1).foldLeft(0f) { case (maxTan, (h, i)) =>
      val newTan = max(maxTan, h / i)
      output(i) = newTan
      newTan
    }
    output(0) = 0.0f
  ...
  def parLineOfSight(
    ...
    val downsweepTree = upsweep(input, 1, input.length, threshold)
    downsweep(input, output, 0.0f, downsweepTree)
    output(0) = 0.0f
  }
}
